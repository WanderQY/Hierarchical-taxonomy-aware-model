    ...
    def train_single_epoch(self, with_center=True):
        ...
        for b, batch in enumerate(self.train_loader):
            ...
            if random.random() <= 0.75:
                ...
                ce_loss = loss_func(self.criterion, outputs)

                if with_center:
                    center_loss = loss_func(self.update_feature_center, multih_fmatrixs)
                ...
                for i, out in enumerate(outputs):
                    batch_corr[i] += lam * (out.argmax(1) == labels_a[:, i]).sum() + (1 - lam) * (
                            out.argmax(1) == labels_b[:, i]).sum()
...
if __name__ == '__main__':
    ...
    config = {'optim': {'name': 'Adam',
              ...
              'scheduler': {'name': 'CosineAnnealingLR',
                            'param': [32, 1e-9]},
              'criterion': {'name': 'HierLoss'}}

    optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-5, eps=1e-3)
    ...
    train_dataset = BirdsoundData(train_datas, class_list=class_list, option='train', augment=['noise', 'distortion', 'cut_mix'])
    ...
    #ckpt_path = '2023-05-06/ckpt/best.pt'
    Train = ModelTrainer(model, train_loader, valid_loader, criterion, optimizer, scheduler, config, hierarchy,
                         dataset_name, epochs, save_path)
